...
import 'package:cirilla/product_interaction.dart';
...
import 'package:cirilla/filters/filter_price.dart';
...
mixin ProductMixin {
    ...
    if (product.type == productTypeVariable || product.type == productTypeGrouped) {
      if (product.priceHtml?.contains('<del') ?? false) {
        return Html(
          ...
          extensions: const [
            HideClassPrice(className: "screen-reader-text"),
          ],
        );
      }
  ...
  Widget? buildQuantityItem(
    ...
    int? max = b2bkingFilterQuantity(context, stockQty != null && stockQty > 0 ? stockQty : CirillaQuantity.maxQty, product, "max");
    int min = b2bkingFilterQuantity(context, 1, product, "min") ?? 1;
    int step = b2bkingFilterQuantity(context, 1, product, "step") ?? 1;
    return CirillaQuantity(
      key: Key("product_quantity_id=${product.id}_value=$quantity"),
      max: max,
      min: min,
      step: step,
      value: quantity,
  ...
  Widget buildAddCart(
    ...
    Icon iconWidget = Icon(icon ?? FeatherIcons.truck, size: 14);
  ...
  Widget buildProgressSale(BuildContext context, {
    Product? product,
    ...
    int count = product?.totalSales ?? 0;
    double rating = 1;
    ...
    if (product?.stockStatus != 'outofstock') {
      if (product?.stockQuantity != null && product!.stockQuantity! > 0) {
        ...
        rating = double.parse((count / total).toStringAsFixed(2));
      } else {
        rating = 0;
      }
    }
